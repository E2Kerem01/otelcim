...
| Yüksek | `lib/shared/providers/paginated_listings_provider.dart:62-74, 101-113` ve `lib/features/home/presentation/home_screen.dart:537-541,605-643` | offline akışı veya Firestore erişim hatası | Beklenen: kullanıcıya ağ hatası ve yeniden dene. Gerçek: hata yalnızca loglanıp boş listeye dönüyor; Home normal boş durumunu ve seed butonunu gösteriyor. `offline/01_offline_feed_error_retry.yaml` doğru davranışı belgeleyen skip akışıdır. |
...
- Bu ortamda cihaz/emülatör ve ağ kesme yetkisi yok; Maestro/ADB çalıştırılmadı. `offline/` akışını orkestratör uçak modu ile çalıştırmalıdır.
